...
void main()
{
    ...
    if (stateVar == 0)
    {
        float _2692 = RVar[19u].w;
        float _2694 = RVar[19u].w;
        RVar[0u].x = dot(RVar[18u], CBUFFER_3.values[3]);
        float _2703 = RVar[19u].z;
        float _2705 = RVar[19u].z;
        float _2707 = (_2703 * _2705) + (_2692 * _2694);
        RVar[0u].y = dot(RVar[18u], CBUFFER_3.values[4]);
        RVar[122u].x = _2707;
        float _2717 = RVar[19u].y;
        float _2719 = RVar[19u].y;
        float _2721 = (_2717 * _2719) + _2707;
        RVar[0u].z = dot(RVar[18u], CBUFFER_3.values[5]);
        RVar[122u].x = _2721;
        float _2731 = RVar[19u].x;
        float _2733 = RVar[19u].x;
        float _2735 = (_2731 * _2733) + _2721;
        RVar[0u].w = dot(RVar[18u], CBUFFER_3.values[6]);
        RVar[122u].x = _2735;
        vec4 _2739 = RVar[18u];
        RVar[127u].y = inversesqrt(_2735);
        RVar[1u].y = CBUFFER_3.values[26].y;
        RVar[1u].x = CBUFFER_3.values[26].x;
        float _2765 = RVar[19u].y;
        float _2767 = RVar[127u].y;
        float _2778 = RVar[19u].z;
        float _2780 = RVar[127u].y;
        RVar[2u].x = RVar[19u].x * RVar[127u].y;
        RVar[2u].y = _2765 * _2767;
        RVar[1u].z = CBUFFER_3.values[26].z;
        RVar[1u].w = clamp(((-dot(_2739, CBUFFER_3.values[2])) + (-CBUFFER_3.values[34].x)) * CBUFFER_3.values[42].x, 0.0, 1.0);
        RVar[2u].z = _2778 * _2780;
    }
    vec4 _2802 = (RVar[0u] * vec4(VS_PUSH.posMulAdd.xy, 1.0, 1.0)) + vec4(VS_PUSH.posMulAdd.zw, 0.0, 0.0);
    vec4 _2805 = _2802;
    _2805.y = -_2802.y;
    vec4 _2813 = _2805;
    _2813.z = (_2805.z + (_2805.w * VS_PUSH.zSpaceMul.x)) * VS_PUSH.zSpaceMul.y;
    gl_Position = _2813;
    PARAM_0 = RVar[1u];
    PARAM_1 = RVar[2u];
    if (stateVar == 0)
