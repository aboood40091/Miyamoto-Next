...
void main()
{
    ...
    if (stateVar == 0)
    {
        ...
        RVar[2u].y = (-(dot(vec4(_2759, _2762, _2765, _2768), CBUFFER_3.values[1]) * 0.5)) + uintBitsToFloat(1056964608u);
        float _2801 = RVar[127u].y;
        float _2803 = RVar[127u].z;
        float _2805 = RVar[126u].x;
        RVar[2u].x = (dot(_2788, CBUFFER_3.values[0]) * 0.5) + uintBitsToFloat(1056964608u);
        RVar[3u].y = _2801;
        RVar[3u].z = _2803;
        RVar[1u].w = clamp(_2805 * CBUFFER_3.values[41].x, 0.0, 1.0);
    }
    vec4 _2831 = (RVar[0u] * vec4(VS_PUSH.posMulAdd.xy, 1.0, 1.0)) + vec4(VS_PUSH.posMulAdd.zw, 0.0, 0.0);
    vec4 _2834 = _2831;
    _2834.y = -_2831.y;
    vec4 _2842 = _2834;
    _2842.z = (_2834.z + (_2834.w * VS_PUSH.zSpaceMul.x)) * VS_PUSH.zSpaceMul.y;
    gl_Position = _2842;
    PARAM_0 = RVar[1u];
    PARAM_1 = RVar[3u];
    if (stateVar == 0)
    {
        RVar[0u].x = dot(vec4(RVar[2u].x, RVar[2u].y, RVar[2u].z, RVar[2u].z), CBUFFER_4.values[17]);
        RVar[0u].y = dot(vec4(RVar[2u].x, RVar[2u].y, RVar[2u].z, RVar[2u].z), CBUFFER_4.values[18]);
