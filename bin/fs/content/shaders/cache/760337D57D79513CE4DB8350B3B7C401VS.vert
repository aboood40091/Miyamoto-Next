...
void main()
{
    ...
    if (stateVar == 0)
    {
        ...
        RVar[127u].z = _2762;
        RVar[127u].w = _2765;
        RVar[126u].x = (-dot(_2748, CBUFFER_3.values[2])) + (-CBUFFER_3.values[34].x);
        RVar[4u].x = _2756;
        vec4 _2785 = RVar[127u];
        RVar[2u].y = (-(dot(vec4(_2756, _2759, _2762, _2765), CBUFFER_3.values[1]) * 0.5)) + uintBitsToFloat(1056964608u);
        float _2802 = RVar[127u].y;
        RVar[2u].x = (dot(_2785, CBUFFER_3.values[0]) * 0.5) + uintBitsToFloat(1056964608u);
        RVar[1u].y = CBUFFER_3.values[26].y;
        RVar[1u].z = CBUFFER_3.values[26].z;
        RVar[4u].y = _2802;
        float _2810 = RVar[126u].x;
        RVar[4u].z = RVar[127u].z;
        RVar[1u].w = clamp(_2810 * CBUFFER_3.values[42].x, 0.0, 1.0);
    }
    vec4 _2834 = (RVar[0u] * vec4(VS_PUSH.posMulAdd.xy, 1.0, 1.0)) + vec4(VS_PUSH.posMulAdd.zw, 0.0, 0.0);
    vec4 _2837 = _2834;
    _2837.y = -_2834.y;
    vec4 _2845 = _2837;
    _2845.z = (_2837.z + (_2837.w * VS_PUSH.zSpaceMul.x)) * VS_PUSH.zSpaceMul.y;
    gl_Position = _2845;
    PARAM_0 = RVar[1u];
    PARAM_1 = RVar[4u];
    if (stateVar == 0)
    {
        RVar[0u].x = dot(vec4(RVar[5u].x, RVar[5u].y, RVar[3u].z, RVar[3u].z), CBUFFER_4.values[17]);
        RVar[0u].y = dot(vec4(RVar[5u].x, RVar[5u].y, RVar[3u].z, RVar[3u].z), CBUFFER_4.values[18]);
        RVar[1u].x = dot(vec4(RVar[5u].x, RVar[5u].y, RVar[3u].z, RVar[3u].z), CBUFFER_4.values[20]);
        RVar[1u].y = dot(vec4(RVar[5u].x, RVar[5u].y, RVar[3u].z, RVar[3u].z), CBUFFER_4.values[21]);
        RVar[3u].x = dot(vec4(RVar[5u].x, RVar[5u].y, RVar[3u].z, RVar[3u].z), CBUFFER_4.values[23]);
        RVar[3u].y = dot(vec4(RVar[5u].x, RVar[5u].y, RVar[3u].z, RVar[3u].z), CBUFFER_4.values[24]);
        RVar[4u].x = dot(vec4(RVar[2u].x, RVar[2u].y, RVar[2u].z, RVar[2u].z), CBUFFER_4.values[26]);
        RVar[4u].y = dot(vec4(RVar[2u].x, RVar[2u].y, RVar[2u].z, RVar[2u].z), CBUFFER_4.values[27]);
    }
    PARAM_2 = vec4(RVar[0u].x, RVar[0u].y, RVar[0u].z, RVar[0u].z);
    PARAM_3 = vec4(RVar[1u].x, RVar[1u].y, RVar[1u].z, RVar[1u].z);
    PARAM_4 = vec4(RVar[3u].x, RVar[3u].y, RVar[3u].z, RVar[3u].z);
