...
void main()
{
    ...
    if (stateVar == 0)
    {
        ...
        RVar[8u].y = (RVar[6u].x * CBUFFER_4.values[18].x) + _3337;
        RVar[1u].z = _3358 * max(_3326, uintBitsToFloat(1065353216u));
        RVar[1u].w = max(clamp(_3311 + CBUFFER_4.values[2].w, 0.0, 1.0), uintBitsToFloat(0u)) * CBUFFER_4.values[0].w;
        RVar[0u].x = dot(vec4(RVar[5u].x, RVar[4u].y, RVar[4u].z, RVar[125u].y), vec4(CBUFFER_4.values[20].x, CBUFFER_4.values[20].y, CBUFFER_4.values[20].z, uintBitsToFloat(1065353216u)));
        RVar[0u].y = dot(vec4(RVar[5u].x, RVar[4u].y, RVar[4u].z, RVar[4u].z), CBUFFER_4.values[21]);
    }
    PARAM_2 = RVar[1u];
